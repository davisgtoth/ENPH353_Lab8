...
if __name__ == '__main__':
    ...
    # define environment from gazebo_env_linefollow.py
    ...
    # define a Q learning class with actions, learning rate, discount rate, 
    # and exploration vs exlpoitation rate
    # also sets the Q matrix to all zeros because Q dictionary is empty
    ...
    for x in range(total_episodes):
        ...
        # start in initial state (I think specified by launch file?)
        observation = env.reset()
        state = ','.join(map(str, observation))

        # render() #defined above, not env.render()

        # go through training episodes until termination condition
        i = -1
        while True:
            i += 1
